...
public class RelayRaceConfigValidator extends ConfigProcessor {
    ...
    public void processConfig() {
        ...
        validateDNFRecords(config.getString(KEY_DNF_FINISHERS), config.getConfigPath().getFileName().toString());
        validateMassStartTimes(config.getString(KEY_MASS_START_TIMES), (int) config.get(KEY_NUMBER_OF_LEGS), config.getConfigPath().getFileName().toString());
    }
            ...
            for (final String individual_dnf_string : dnf_string.split(SEPARATOR_CONFIG_OUTER)) {
                try {
                    ...
                    if (elements.length < 2)
                        throw new RuntimeException(ERROR_CONFIG_ENTRY_INVALID + " '" + dnf_string + "' " + ERROR_CONFIG_KEY + " '" + KEY_DNF_FINISHERS + "' " + ERROR_FILE + " '" + config_file_name + "'");
    ...
    static void validateMassStartTimes(final String mass_start_elapsed_times, final int number_of_legs, final String config_file_name) {
        ...
        if (mass_start_elapsed_times != null)
            for (final String leg_time_string : mass_start_elapsed_times.split(SEPARATOR_CONFIG_OUTER)) {
                ...
                try {
                    if (split.length < 2)
                        throw new RuntimeException(ERROR_MASS_START_TIME_INVALID + " '" + KEY_MASS_START_TIMES + "' " + ERROR_FILE + " '" + config_file_name + "'");
                ...
                try {
                    ...
                    if (leg_number <= 1 || leg_number > number_of_legs)
                        throw new RuntimeException(ERROR_LEG_NUMBER_INVALID + " '" + KEY_MASS_START_TIMES + "' " + ERROR_FILE + " '" + config_file_name + "'");
                }
                ...
                if (previous_time != null && previous_time.compareTo(mass_start_time) >= 0)
                    throw new RuntimeException(ERROR_MASS_START_ORDER_INVALID + " '" + KEY_MASS_START_TIMES + "' " + ERROR_FILE + " '" + config_file_name + "'");
