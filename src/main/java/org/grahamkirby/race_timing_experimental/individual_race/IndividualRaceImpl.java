...
public class IndividualRaceImpl implements SpecificRace {
    ...
    private record SeparatelyRecordedResult(int bib_number, Duration finish_time) {
    }
    ...
    private Map<EntryCategory, Duration> readCategoryStartOffsets() {
        ...
        if (race.getConfig().containsKey("CATEGORY_START_OFFSETS")) {
            ...
            for (final String offset_string : offset_strings) {
                ...
                category_offsets.put(race.getCategoryDetails().lookupEntryCategory(split[0]), parseTime(split[1]));
            }
        }
