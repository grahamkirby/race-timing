...
public class RelayRaceOutputCSV {
    ...
    void printResults() throws IOException {
        ...
        try (final OutputStreamWriter writer = new OutputStreamWriter(stream)) {
            ...
            writer.append(STR."\{OVERALL_RESULTS_HEADER}Total\{Config.LINE_SEPARATOR}");
    ...
    void printDetailedResults() throws IOException {
        ...
        try (final OutputStreamWriter writer = new OutputStreamWriter(Files.newOutputStream(race.getOutputDirectoryPath().resolve(STR."\{race_name}_detailed_\{year}.\{Config.CSV_FILE_SUFFIX}"), Config.STANDARD_FILE_OPEN_OPTIONS))) {
    ...
    private void printDetailedResultsHeader(final OutputStreamWriter writer) throws IOException {
        ...
        writer.append("Total").append(Config.LINE_SEPARATOR);
    }
    ...
    private void printLegResults(final int leg) throws IOException {
        ...
        try (final OutputStreamWriter writer = new OutputStreamWriter(Files.newOutputStream(race.getOutputDirectoryPath().resolve(STR."\{race_name}_\{output_type}_\{year}.\{Config.CSV_FILE_SUFFIX}"), Config.STANDARD_FILE_OPEN_OPTIONS))) {
    ...
    private static final class OverallResultPrinter extends ResultPrinter {
        ...
        public void printResult(final RaceResult r) throws IOException {
            ...
            writer.append(STR."\{result.position_string},\{result.entry.bib_number},\{Config.encode(result.entry.participant.name)},\{result.entry.participant.category.getShortName()},\{Config.renderDuration(result, Config.DNF_STRING)}\n");
        }
    }
    ...
    private static final class DetailedResultPrinter extends ResultPrinter {
        ...
        public void printResult(final RaceResult r) throws IOException {
            ...
            writer.append(STR."\{result.position_string},\{result.entry.bib_number},\{Config.encode(result.entry.participant.name)},\{result.entry.participant.category.getLongName()},");
    ...
    private static final class LegResultPrinter extends ResultPrinter {
        ...
        public void printResult(final RaceResult r) throws IOException {
            ...
            final String runner_names = Config.encode(((Team) result.entry.participant).runner_names.get(result.leg_number - 1));

            writer.append(STR."\{result.position_string},\{runner_names},\{Config.renderDuration(result, Config.DNF_STRING)}\n");
        }
    }
}
