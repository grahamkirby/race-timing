...
public class RaceTest {
    ...
    //
    // If the property is defined then output files are generated within the specified directory. If the
    ...
    public RaceTest() throws IOException {
        ...
        final Path overall_test_config_path = getPathRelativeToProjectRoot("/src/test/resources/").resolve("test_config.txt");

        final Properties overall_test_properties = loadProperties(overall_test_config_path);

        user_test_directory_path_string = (String) overall_test_properties.get("USER_TEST_DIRECTORY_PATH");

        debug = user_test_directory_path_string != null;
        test_directory = getTempDirectory();
        retain_first_output = Boolean.parseBoolean((String) overall_test_properties.get("RETAIN_FIRST_OUTPUT"));
        if (Boolean.parseBoolean((String) overall_test_properties.get("ALWAYS_CHECK_UNUSED_INPUT_FILES")))
            override_check_input_files_used = true;
